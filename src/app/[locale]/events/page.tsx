    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (data.event && Array.isArray(data.event)) {
          const formattedEvents = data.event.map((event) => ({
            id: event.id,
            imageUrl: event.image_url || "/assets/img/press_img.png", // Fallback image
            title: event.title,
            date: event.date,
            link: event.slug,
          }))
                    ...
                    <div className="press_grid_items" key={item.id}>
                      ...
                      <div className="pres_grid_des">
                        ...
                        <div className="press_con">
                          <Link className="text-decoration-none" href={`${LanguageRoute}events/${item.link}`}>
                            <h4>{item.title}</h4>
                          </Link>
                          <div className="schedule_con">
                            ...
                            <Link href={`${LanguageRoute}events/${item.link}`} className="read_more text-decoration-none">
                              Read more &gt;&gt;
                            </Link>
                          </div>
                        </div>
